...
static char gfPrototxtPath[MAX_PATH_SIZE];
static char gfCaffeModelPath[MAX_PATH_SIZE];


int loadPrototxt(const char *fPath) {
    strncpy(gfPrototxtPath, fPath, sizeof(gfPrototxtPath)-1);
    gfPrototxtPath[sizeof(gfPrototxtPath)-1] = '\0';
    return 0;
}

int loadCaffeModel(const char *fPath) {
    strncpy(gfCaffeModelPath, fPath, sizeof(gfCaffeModelPath)-1);
    gfCaffeModelPath[sizeof(gfCaffeModelPath)-1] = '\0';
    return 0;
}
...
int classify(unsigned char *inImg, int *info) {
    ...
    /* Load Prototxt */
    caffe::Net<float> caffe_test_net(gfPrototxtPath, caffe::TEST);

    /* Load training data */
    caffe_test_net.CopyTrainedLayersFrom(gfCaffeModelPath);
    ...
    LOGI("classified %F correctly in class 0\n", static_cast<double>(iCorrectPixelsClass0) / iPixelsClass0 * 100);
    LOGI("classified %F correctly in class 1\n", static_cast<double>(iCorrectPixelsClass1) / iPixelsClass1 * 100);
    LOGI("classified %F correctly in class 2\n", static_cast<double>(iCorrectPixelsClass2) / iPixelsClass2 * 100);
    LOGI("classified %F in total correctly\n", static_cast<double>(iCorrectPixelsClass0 + iCorrectPixelsClass1 + iCorrectPixelsClass2) / iProcessedPixels * 100);
